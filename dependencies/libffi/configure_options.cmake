...
if(TARGET_PLATFORM STREQUAL X86_WIN64)
    if(MSVC)
        list(APPEND WIN_ASSEMBLY_LIST ${LIBFFI_PATH}/src/x86/win64_intel.S)
        enable_language(ASM_MASM)
    else()
        list(APPEND SOURCES_LIST ${LIBFFI_PATH}/src/x86/win64.S)
    endif()
    list(APPEND SOURCES_LIST ${LIBFFI_PATH}/src/x86/ffiw64.c)

    set(TARGETDIR x86)
elseif(TARGET_PLATFORM STREQUAL X86_64)
    list(APPEND SOURCES_LIST
        ${LIBFFI_PATH}/src/x86/ffi64.c
        ${LIBFFI_PATH}/src/x86/unix64.S)
    ...
    if(NOT TARGET_X32)
        list(APPEND SOURCES_LIST
             ${LIBFFI_PATH}/src/x86/ffiw64.c
             ${LIBFFI_PATH}/src/x86/win64.S)
    endif()

    set(TARGETDIR x86)
elseif(TARGET_PLATFORM MATCHES X86.*)
    if(MSVC)
        list(APPEND WIN_ASSEMBLY_LIST ${LIBFFI_PATH}/src/x86/sysv_intel.S)
        set (CMAKE_SHARED_LINKER_FLAGS "${CMAKE_SHARED_LINKER_FLAGS} /SAFESEH:NO")
        enable_language(ASM_MASM)
    else()
        list(APPEND SOURCES_LIST ${LIBFFI_PATH}/src/x86/sysv.S)
    endif()

    list(APPEND SOURCES_LIST ${LIBFFI_PATH}/src/x86/ffi.c)
    set(TARGETDIR x86)
elseif(TARGET_PLATFORM MATCHES ARM_WIN64|AARCH64)
    if(TARGET_PLATFORM STREQUAL ARM_WIN64)
        ...
        list(APPEND WIN_ASSEMBLY_LIST ${LIBFFI_PATH}/src/aarch64/win64_armasm.S)
        file(COPY ${LIBFFI_PATH}/src/aarch64/ffitarget.h DESTINATION ${LIBFFI_BINARY_DIR}/include)
        enable_language(ASM)
    else()
        list(APPEND SOURCES_LIST ${LIBFFI_PATH}/src/aarch64/sysv.S)
    endif()

    list(APPEND SOURCES_LIST ${LIBFFI_PATH}/src/aarch64/ffi.c)
    set(TARGETDIR aarch64)
elseif(TARGET_PLATFORM MATCHES ARM.*)
    if(MSVC)
        set(CMAKE_ASM_MASM_COMPILER ${COMPILER_DIR}/armasm.exe)
        set(CMAKE_ASM_COMPILER ${CMAKE_ASM_MASM_COMPILER})
        list(APPEND WIN_ASSEMBLY_LIST ${LIBFFI_PATH}/src/arm/sysv_msvc_arm32.S)
        file(COPY ${LIBFFI_PATH}/src/arm/ffitarget.h DESTINATION ${LIBFFI_BINARY_DIR}/include)
        enable_language(ASM)
    else()
        list(APPEND SOURCES_LIST ${LIBFFI_PATH}/src/arm/sysv.S)
    endif()

    list(APPEND SOURCES_LIST ${LIBFFI_PATH}/src/arm/ffi.c)
    set(TARGETDIR arm)
endif()
...
if (NOT MSVC)
    ...
    file(REMOVE ${LIBFFI_BINARY_DIR}/conftest.*)
...
configure_file(${LIBFFI_PATH}/include/ffi.h.in ${LIBFFI_BINARY_DIR}/include/ffi.h)
configure_file(cmake_fficonfig.h.in ${LIBFFI_BINARY_DIR}/fficonfig.h)
...
foreach(ASM_PATH IN LISTS WIN_ASSEMBLY_LIST)
    ...
    add_custom_command(
        COMMAND "${CMAKE_C_COMPILER}" /nologo /P /EP /I. /I"${CMAKE_CURRENT_SOURCE_DIR}/${ASM_DIRNAME}" /Fi"${LIBFFI_BINARY_DIR}/${ASM_FILENAME}.asm" /Iinclude
                /I"${LIBFFI_BINARY_DIR}" /I"${LIBFFI_BINARY_DIR}/include" /I"${CMAKE_CURRENT_SOURCE_DIR}/libffi/include" "${CMAKE_CURRENT_SOURCE_DIR}/${ASM_PATH}"
        DEPENDS ${CMAKE_CURRENT_SOURCE_DIR}/${ASM_PATH}
        OUTPUT ${LIBFFI_BINARY_DIR}/${ASM_FILENAME}.asm
        COMMENT "Preprocessing ${CMAKE_CURRENT_SOURCE_DIR}/${ASM_PATH}. Outputting to ${LIBFFI_BINARY_DIR}/${ASM_FILENAME}.asm")
