...
namespace RHook {
	...
	public:
		...
		void Map(VkInstance hInstance, const char* pCommandName, Address icdCommand, Address detour = nullptr) {
			// ICD Command to instance handle map
			{
				if (!m_ICDCommandToInstanceHandleMap.contains(icdCommand)) {
					m_ICDCommandToInstanceHandleMap.insert({ icdCommand, hInstance });
				}
			}
			...
			if (detour != nullptr) {
				...
				if (!instanceMap.contains(pCommandName)) {
					instanceMap.insert({ pCommandName, std::make_unique<FunctionHook>(icdCommand, detour) });
				}
			}
		...
		std::optional<std::reference_wrapper<std::unique_ptr<FunctionHook>>> GetCommandHookFromInstanceHandle(VkInstance hInstance, const char* pName) {
			if (!m_InstanceHandleToICDCommandHooksMap.contains(hInstance)) {
				return {};
			}
	...
	private:
		...
		std::unordered_map<VkInstance, std::unordered_map<std::string_view, std::unique_ptr<FunctionHook>>> m_InstanceHandleToICDCommandHooksMap{};
	...
	public:
		...
		void Map(VkDevice hDevice, const char* pCommandName, Address icdCommand, Address detour = nullptr) {
			// ICD Command to device handle map
			{
				if (!m_ICDCommandToDeviceHandleMap.contains(icdCommand)) {
					m_ICDCommandToDeviceHandleMap.insert({ icdCommand, hDevice });
				}
			}
			...
			if (detour != nullptr) {
				...
				if (!deviceMap.contains(pCommandName)) {
					deviceMap.insert({ pCommandName, std::make_unique<FunctionHook>(icdCommand, detour) });
				}
			}
		...
		std::optional<std::reference_wrapper<std::unique_ptr<FunctionHook>>> GetCommandHookFromDeviceHandle(VkDevice hDevice, const char* pName) {
			if (!m_DeviceHandleToICDCommandHooksMap.contains(hDevice)) {
				return {};
			}
	...
	private:
		...
		std::unordered_map<VkDevice, std::unordered_map<std::string_view, std::unique_ptr<FunctionHook>>> m_DeviceHandleToICDCommandHooksMap{};
...
#define ADD_RESULT_LOGIC(TYPE, SYMBOL) auto& Get##SYMBOL##Result() { assert(s_ResultList[(size_t)HIdx::SYMBOL] != nullptr, "Tried to access hook result outside call!"); \
	...
	protected:
		...
		// ICD Commands... oof...
		//static VKAPI_ATTR VkResult ICDVkCreateInstance(const VkInstanceCreateInfo* pCreateInfo, const VkAllocationCallbacks* pAllocator, VkInstance* pInstance);
		//static VKAPI_ATTR VkResult VKAPI_CALL ICDVkCreateDevice(VkPhysicalDevice physicalDevice, const VkDeviceCreateInfo* pCreateInfo, const VkAllocationCallbacks* pAllocator, VkDevice* pDevice);
		static VKAPI_ATTR VkResult VKAPI_CALL ICDVkCreateFence(VkDevice device, const VkFenceCreateInfo* pCreateInfo, const VkAllocationCallbacks* pAllocator, VkFence* pFence);
		static VKAPI_ATTR VkResult VKAPI_CALL ICDVkCreateSemaphore(VkDevice device, const VkSemaphoreCreateInfo* pCreateInfo, const VkAllocationCallbacks* pAllocator, VkSemaphore* pSemaphore);
		static VKAPI_ATTR VkResult VKAPI_CALL ICDVkCreateEvent(VkDevice device, const VkEventCreateInfo* pCreateInfo, const VkAllocationCallbacks* pAllocator, VkEvent* pEvent);
		static VKAPI_ATTR VkResult VKAPI_CALL ICDVkCreateQueryPool(VkDevice device, const VkQueryPoolCreateInfo* pCreateInfo, const VkAllocationCallbacks* pAllocator, VkQueryPool* pQueryPool);
		static VKAPI_ATTR VkResult VKAPI_CALL ICDVkCreateBuffer(VkDevice device, const VkBufferCreateInfo* pCreateInfo, const VkAllocationCallbacks* pAllocator, VkBuffer* pBuffer);
		static VKAPI_ATTR VkResult VKAPI_CALL ICDVkCreateBufferView(VkDevice device, const VkBufferViewCreateInfo* pCreateInfo, const VkAllocationCallbacks* pAllocator, VkBufferView* pView);
		static VKAPI_ATTR VkResult VKAPI_CALL ICDVkCreateImage(VkDevice device, const VkImageCreateInfo* pCreateInfo, const VkAllocationCallbacks* pAllocator, VkImage* pImage);
		static VKAPI_ATTR VkResult VKAPI_CALL ICDVkCreateImageView(VkDevice device, const VkImageViewCreateInfo* pCreateInfo, const VkAllocationCallbacks* pAllocator, VkImageView* pView);
		static VKAPI_ATTR VkResult VKAPI_CALL ICDVkCreateGraphicsPipelines(VkDevice device, VkPipelineCache pipelineCache, uint32_t createInfoCount, const VkGraphicsPipelineCreateInfo* pCreateInfos, const VkAllocationCallbacks* pAllocator, VkPipeline* pPipelines);
		static VKAPI_ATTR VkResult VKAPI_CALL ICDVkCreateFramebuffer(VkDevice device, const VkFramebufferCreateInfo* pCreateInfo, const VkAllocationCallbacks* pAllocator, VkFramebuffer* pFramebuffer);
		static VKAPI_ATTR VkResult VKAPI_CALL ICDVkCreateDescriptorPool(VkDevice device, const VkDescriptorPoolCreateInfo* pCreateInfo, const VkAllocationCallbacks* pAllocator, VkDescriptorPool* pDescriptorPool);
		static VKAPI_ATTR VkResult VKAPI_CALL ICDVkCreateWin32SurfaceKHR(VkInstance instance, const VkWin32SurfaceCreateInfoKHR* pCreateInfo, const VkAllocationCallbacks* pAllocator, VkSurfaceKHR* pSurface);
		static VKAPI_ATTR VkResult VKAPI_CALL ICDVkCreateSwapchainKHR(VkDevice device, const VkSwapchainCreateInfoKHR* pCreateInfo, const VkAllocationCallbacks* pAllocator, VkSwapchainKHR* pSwapchain);
		static VKAPI_ATTR VkResult VKAPI_CALL ICDVkCreateRenderPass(VkDevice device, const VkRenderPassCreateInfo* pCreateInfo, const VkAllocationCallbacks* pAllocator, VkRenderPass* pRenderPass);
		
		static VKAPI_ATTR VkResult VKAPI_CALL ICDVkAllocateCommandBuffers(VkDevice device, const VkCommandBufferAllocateInfo* pAllocateInfo, VkCommandBuffer* pCommandBuffers);
		static VKAPI_ATTR VkResult VKAPI_CALL ICDVkBeginCommandBuffer(VkCommandBuffer commandBuffer, const VkCommandBufferBeginInfo* pBeginInfo);
		static VKAPI_ATTR void VKAPI_CALL ICDVkCmdBeginRenderPass(VkCommandBuffer commandBuffer, const VkRenderPassBeginInfo* pRenderPassBegin, VkSubpassContents contents);

		static VKAPI_ATTR void VKAPI_CALL ICDVkGetDeviceQueue(VkDevice device, uint32_t queueFamilyIndex, uint32_t queueIndex, VkQueue* pQueue);
		static VKAPI_ATTR VkResult VKAPI_CALL ICDVkAcquireNextImageKHR(VkDevice device, VkSwapchainKHR swapchain, uint64_t timeout, VkSemaphore semaphore, VkFence fence, uint32_t* pImageIndex);
		static VKAPI_ATTR VkResult VKAPI_CALL ICDVkQueueSubmit(VkQueue queue, uint32_t submitCount, const VkSubmitInfo* pSubmits, VkFence fence);
		static VKAPI_ATTR VkResult VKAPI_CALL ICDVkQueuePresentKHR(VkQueue queue, const VkPresentInfoKHR* pPresentInfo);
		
		static VKAPI_ATTR void VKAPI_CALL ICDVkCmdEndRenderPass(VkCommandBuffer commandBuffer);
		static VKAPI_ATTR VkResult VKAPI_CALL ICDVkEndCommandBuffer(VkCommandBuffer commandBuffer);
		static VKAPI_ATTR void VKAPI_CALL ICDVkFreeCommandBuffers(VkDevice device, VkCommandPool commandPool, uint32_t commandBufferCount, const VkCommandBuffer* pCommandBuffers);

		static VKAPI_ATTR void VKAPI_CALL ICDVkDestroyRenderPass(VkDevice device, VkRenderPass renderPass, const VkAllocationCallbacks* pAllocator);
		static VKAPI_ATTR void VKAPI_CALL ICDVkDestroySwapchainKHR(VkDevice device, VkSwapchainKHR swapchain, const VkAllocationCallbacks* pAllocator);
		static VKAPI_ATTR void VKAPI_CALL ICDVkDestroySurfaceKHR(VkInstance instance, VkSurfaceKHR surface, const VkAllocationCallbacks* pAllocator);
		static VKAPI_ATTR void VKAPI_CALL ICDVkDestroyDescriptorPool(VkDevice device, VkDescriptorPool descriptorPool, const VkAllocationCallbacks* pAllocator);
		static VKAPI_ATTR void VKAPI_CALL ICDVkDestroyFramebuffer(VkDevice device, VkFramebuffer framebuffer, const VkAllocationCallbacks* pAllocator);
		static VKAPI_ATTR void VKAPI_CALL ICDVkDestroyPipeline(VkDevice device, VkPipeline pipeline, const VkAllocationCallbacks* pAllocator);
		static VKAPI_ATTR void VKAPI_CALL ICDVkDestroyImageView(VkDevice device, VkImageView imageView, const VkAllocationCallbacks* pAllocator);
		static VKAPI_ATTR void VKAPI_CALL ICDVkDestroyImage(VkDevice device, VkImage image, const VkAllocationCallbacks* pAllocator);
		static VKAPI_ATTR void VKAPI_CALL ICDVkDestroyBufferView(VkDevice device, VkBufferView bufferView, const VkAllocationCallbacks* pAllocator);
		static VKAPI_ATTR void VKAPI_CALL ICDVkDestroyBuffer(VkDevice device, VkBuffer buffer, const VkAllocationCallbacks* pAllocator);
		static VKAPI_ATTR void VKAPI_CALL ICDVkDestroyQueryPool(VkDevice device, VkQueryPool queryPool, const VkAllocationCallbacks* pAllocator);
		static VKAPI_ATTR void VKAPI_CALL ICDVkDestroyEvent(VkDevice device, VkEvent event, const VkAllocationCallbacks* pAllocator);
		static VKAPI_ATTR void VKAPI_CALL ICDVkDestroySemaphore(VkDevice device, VkSemaphore semaphore, const VkAllocationCallbacks* pAllocator);
		static VKAPI_ATTR void VKAPI_CALL ICDVkDestroyFence(VkDevice device, VkFence fence, const VkAllocationCallbacks* pAllocator);
		static VKAPI_ATTR void VKAPI_CALL ICDVkDestroyDevice(VkDevice device, const VkAllocationCallbacks* pAllocator);
		static VKAPI_ATTR void VKAPI_CALL ICDVkDestroyInstance(VkInstance instance, const VkAllocationCallbacks* pAllocator);
